...
SELECT 
	tt.twtid,
	tt.userid AS userid,
    tu.username AS userhandle,
    tu.givenname AS username,
    tu.user_lang,
    tg.geo AS user_geo,
    tun.numfriends AS usernumfriends,
	tun.numfollowers usernumfollowers,
	tun.time_stamp AS userinfo_lastupdated,
	tt.twttext,
	tt.twt_createdat,
	tt.twt_lang,
    tn.favcount,
...
/*View to display today's top hashtags*/
DROP VIEW VD_todays_top_hashtags;

CREATE VIEW VD_todays_top_hashtags AS
SELECT 
	h2.hashtag,
    COUNT(h1.hashid) AS num_used
FROM VDS_todays_twtids todays_twtids
	INNER JOIN meta_hashmaster1 h1
    ON todays_twtids.twtid=h1.twtid
    
    INNER JOIN meta_hashmaster2 h2
    ON h1.hashid=h2.hashid
